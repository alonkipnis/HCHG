...
def sample_survival_poisson(T, N1, N2, lam0, eps, r):
    ...
    # if lam0 is a float or int, make it into an array of length T
...
def sample_survival_poisson_proportional(T, N1, N2, lam1, lam2):
    """
    Proportional sampling of two Poisson distributions.
    ...
    N1   total in group 1 at t=0
    N2   total in group 2 at t=0
    lam1 Poisson rate for group 1
    lam2 Poisson rate for group 2

    Note that since we sample from two Poisson distributions
    in each 'event', there is some possibility that we draw (O1,O2) = (0,0),
    hence there is no change in that event. This situation is different from standard notation.
    ...
    for t in np.arange(T):
        O1 = poisson.rvs(Nt1[t] * lam1 * (Nt1[t] > 0))
        O2 = poisson.rvs(Nt2[t] * lam2 * (Nt2[t] > 0))
        Nt1[t + 1] = np.maximum(Nt1[t] - O1, 0)
        Nt2[t + 1] = np.maximum(Nt2[t] - O2, 0)
...
def sample_survival_poisson_proportional_censorship(T, N1, N2, lam1, lam2, lamc):
    """
    Proportional sampling of two Poisson distributions with censorship.
    ...
    for t in np.arange(T):
        ...
        O1 = poisson.rvs(Nt1[t] * lam1 * (Nt1[t] > 0))
        O2 = poisson.rvs(Nt2[t] * lam2 * (Nt2[t] > 0))
        # Sample censored events
        C1 = poisson.rvs(Nt1[t] * lamc * (Nt1[t] > 0))
        C2 = poisson.rvs(Nt2[t] * lamc * (Nt2[t] > 0))
        # Make sure we don't remove more than at risk
        total1 = O1 + C1
        total2 = O2 + C2
        if total1 > Nt1[t]:
            # scale down proportionally
            if total1 > 0:
                O1 = int(np.floor(O1 * Nt1[t] / total1))
                C1 = int(Nt1[t] - O1)
            else:
                O1 = 0
                C1 = 0
        if total2 > Nt2[t]:
            if total2 > 0:
                O2 = int(np.floor(O2 * Nt2[t] / total2))
                C2 = int(Nt2[t] - O2)
            else:
                O2 = 0
                C2 = 0
        ...
        Ct1[t] = C1
        Ct2[t] = C2
        Nt1[t + 1] = np.maximum(Nt1[t] - O1 - C1, 0)
        Nt2[t + 1] = np.maximum(Nt2[t] - O2 - C2, 0)

    return Nt1, Nt2, Ot1, Ot2, Ct1, Ct2