...
\begin{itemize}
  ...
  \item HCHG: $p \leq 0.013$ in all four; log-rank and four weighted
        alternatives: $p \geq 0.13$ in all four.
\end{itemize}
...
\begin{itemize}
  ...
  \item HCHG: $p = 0.002$ (significant); Fisher: $p = 0.031$
...
\begin{itemize}
  \item HCHG achieves $p \leq 0.013$ in all four domains;
        log-rank and four weighted alternatives: $p \geq 0.13$ everywhere.
...
\begin{figure*}[t!]
  ...
  \caption{%
    ...
    (shaded). Bottom row (E--H): per-interval
    $-\!\log_{10}$(hypergeometric p-value); bars exceeding the HC threshold
    (dashed, permutation-calibrated) are highlighted in red.
    %
    ...
    log-rank $p=0.262$, HCHG $p=0.012$. Early OS advantage (months~3--7)
    ...
    In all four datasets the log-rank test and four weighted alternatives
    are non-significant. For the first three, IPD reconstructed
    via \citet{guyot2012}.%
  }
  \label{fig:main}
\end{figure*}
